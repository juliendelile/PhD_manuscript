...
class Convertor {    
    ...
    */    
    public void anchorStart(ElementStart e) throws IOException, NoItemException {
        ...
        //System.out.println("href "+ e.getAttributes().get("href") + "   name " +e.getAttributes().get("name")+"  title "+e.getAttributes().get("title"));
    ...
    */       
    public void imgStart(ElementStart es)
        ...
        //modif julien
        //System.out.println("width "+es.getAttributes().get("width")+"  height "+es.getAttributes().get("height"));
        
        //_writer.write("\n\\begin{figure}");
        _writer.write("\n\\begin{center}");
        
        
        //compute width ratio with 1000px width
        
        Double widthlatex;
        
        if( es.getAttributes().get("width") != null ){
        	Double widthhtml = new Double(Double.parseDouble(es.getAttributes().get("width")));
        	widthlatex = widthhtml / 1000;
        	
        	
        	BigDecimal bd = new BigDecimal(widthlatex);
    		BigDecimal rounded = bd.setScale(2, BigDecimal.ROUND_HALF_UP);
    		widthlatex = rounded.doubleValue();
        	
        }
        else{
        	widthlatex = new Double(800 / 1000);
        }
        
       
        
        //_writer.write("\n\\includegraphics{" + es.getAttributes().get("src") + "}");
        _writer.write("\n\\includegraphics[width=" + widthlatex.toString() + "\\textwidth]{" 
    ...
    //modif julien
    //new function
    public void objectStart(ElementStart es)
        throws IOException, NoItemException {
       ...
       String dataname = es.getAttributes().get("data");
       
      	dataname = dataname.substring(0, dataname.length() - 4);
